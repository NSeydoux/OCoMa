...
public class TestEndpoint {
	...
	public Response getKB(){
		Model m = OCoMaModel.getInstance().dumpModel();
...
//			b.getAuthors().add("http://example.com/ns#unAutreAuteur");
	...
	@GET
	@Path("/fetch")
	public Response fetchEntity(){
		Resource r = null;
		try{
			r=OCoMaModel.getInstance().fetchEntity("http://example.com/ns#testComic");
		} catch (Exception e){
			e.printStackTrace();
		}	
		return  Response.status(HttpURLConnection.HTTP_OK)
                .entity("Fetched entity : "+r.toString()).build();
	}
}
